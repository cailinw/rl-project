...
class Rollout():
	...
	def sample_from_policy(self, batch_size, generated_num):
		...
		return self.generator.generate(
			batch_size, generated_num // batch_size,
			inc_hidden_state=False, inc_probs=True, decode=False
		)
	...
	def compute_rewards_to_go(self, trajectories, roll_num):
		...
		for t in range(self.seq_length):
			...
			for n in range(roll_num):
				# TODO: rollout trajectories from s_t to s_{seq_length} to get rollouts (batch_size, seq_length)
				...
				rewards = self.rewarder.compute_rewards(rollouts)  # (batch_size, seq_length)
