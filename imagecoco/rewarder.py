...
class Rewarder():
	...
	def train_step(self, x_real, generator):
		...
		hidden_states_gen, probs, x_gen = generator.generate(num_gen, 1,
			inc_hidden_state = True, inc_probs = True
		)
		...
		for seq in range(num_gen):
			...
			for t in range(self.seq_length):
				...
				R_gen += self.model(hidden_states_gen[seq][t]).detach().numpy()[x_gen[seq][t]] # TODO: x_gen[seq][t] probably not right # Get the probability for the token that was generated
		...
		for seq in range(num_real):
			...
			for t in range(self.seq_length):
				...
				R_gen += self.model(hidden_states_real[seq][t]).detach().numpy()[x_real[seq][t]] # TODO: x_gen[seq][t] probably not right # Get the probability for the token that was generated
...
class Rewarder:
    ...
    def train_step(self, x_text, generator):
        ...
        # TODO: Compute weight
        real_weight = np.array(
            [1.0 / (self.batch_size // 2)] * (self.batch_size // 2)
        )  # N
